...
async function buscarHistorico(telefone, limite) {
  if (!limite) limite = 20;
  const result = await supabase.from('conversas').select('role, conteudo').eq('telefone', telefone).order('created_at', { ascending: true }).limit(limite);
  return (result.data || []).map(function(m) { return { role: m.role, content: m.conteudo }; });
}
...
async function salvarAgendamento(dados) {
  await supabase.from('pacientes').upsert({ nome: dados.nome_paciente, telefone: dados.telefone, convenio: dados.convenio }, { onConflict: 'telefone' });
  const result = await supabase.from('agendamentos').insert({ nome_paciente: dados.nome_paciente, telefone: dados.telefone, especialidade: dados.especialidade, convenio: dados.convenio || 'particular', periodo: dados.periodo, medico: dados.medico || null, status: 'pendente', origem: dados.origem || 'whatsapp' }).select().single();
...
async function buscarMetricas() {
  ...
  return { total: r1.count || 0, hoje: r2.count || 0, confirmados: r3.count || 0, pendentes: r4.count || 0, cancelados: r5.count || 0 };
}
module.exports = { supabase: supabase, buscarHistorico: buscarHistorico, salvarMensagem: salvarMensagem, salvarAgendamento: salvarAgendamento, buscarAgendamentos: buscarAgendamentos, atualizarStatus: atualizarStatus, logMensagem: logMensagem, buscarMetricas: buscarMetricas };
